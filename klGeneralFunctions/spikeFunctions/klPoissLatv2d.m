...
%
%  v1 - Working as of 8/3/15
%  v2 - Trying to use the same logic as before to make this a bidirectional
%       function. That is, you can give it a starting time and have it
%       search forward (i.e., find a burst after an event - default) or
%       give an ending time and have it search backwards (i.e., find a
%       burst preceding an event).
%
...
function [modLat, activeTimes] = klPoissLatv2d(spiketimes,varargin)
% Set some defaults
sigAlph = .05;
checkAlph = .05;
visualize = 0;
...
for iv = 1:length(varStrInd),
    switch varargin{varStrInd(iv)}
        ...
        case '-stop'
            stopTime  = varargin{varStrInd(iv)+1};
        case '-fix'
            fixTime  = varargin{varStrInd(iv)+1};
    end
end
...
if length(stopTime) == 1,
    stopTime = repmat(stopTime,size(spiketimes,1),1);
elseif length(stopTime) ~= size(spiketimes,1),
    error('Size mismatch: length(stopTime) ~= size(spiketimes,1)');
end
...
% Set event for direction checking
if ismember(lower(direction),{'reverse','backward','back','rev'}),
    evTime   = -stopTime;
else
    evTime   = stimTime;
end
...
%     trMeanRate(it) = sum(thisTrSpks < stimTime(it))/(stimTime(it)-fixTime(it));
    ...
    if ismember(lower(direction),{'reverse','backward','back','rev'}),
        thisTrSpks = -fliplr(thisTrSpks);
    end
    
    % Index to first spike after target presentation
    firstSpkInd = find(thisTrSpks >= evTime(it),1);
...
%         [~,checkP] = getSI(burstEndInd-(is-1),trMeanRate(it)*(thisTrSpks(burstEndInd)-thisTrSpks(is)));
        ...
        if visualize > 1,
            figure(1); clf;
            scatter(thisTrSpks,ones(1,length(thisTrSpks)),'r*');
            vline(thisTrSpks(min([is+(nCheckSpks+1),length(thisTrSpks)])));
            vline(thisTrSpks(is));
            t=title(sprintf('p = %.3f',checkP));
            keyboard
        end
...
%         [~,checkP] = getSI(is-(burstStartInd-1),trMeanRate(it)*(thisTrSpks(is)-thisTrSpks(burstStartInd)));
        ...
        if visualize > 1,
            figure(1); clf;
            scatter(thisTrSpks,ones(1,length(thisTrSpks)),'r*');
            vline(thisTrSpks(max([is-(nCheckSpks-1),1])));
            vline(thisTrSpks(is));
            t=title(sprintf('p = %.3f',checkP));
            keyboard
        end
    ...
    if ismember(lower(direction),{'reverse','backward','back','rev'}),
        startTime = -thisTrSpks(postCheckStartInd);
        endTime   = -thisTrSpks(postCheckEndInd);
        if endTime < -cutTime,
            startTime = nan;
            endTime = nan;
            activeTimes(it,:) = [nan,nan];
            activeInds(it,:)  = [nan,nan];
        else
            activeTimes(it,:) = [endTime,startTime]; % Order reversed because of scan direction
            % Re-reverse thisTrSpks
            thisTrSpks = -fliplr(thisTrSpks);
            activeInds(it,1) = find(thisTrSpks == endTime,1,'last');
            activeInds(it,2) = find(thisTrSpks == startTime,1,'first');
        end
    else
        startTime = thisTrSpks(postCheckStartInd);
        endTime   = thisTrSpks(postCheckEndInd);
        if startTime > cutTime,
            ...
            activeTimes(it,:) = [startTime,endTime];
        end
    end
...
if visualize,
    ...
    set(gca,'XLim',[sdfTimes(1),sdfTimes(end)]);
    subplot(2,1,2); hold on;
    for it = 1:size(activeTimes,1),
        if sum(~isnan(activeTimes(it,:))) == 2,
            plot(activeTimes(it,:),[it,it],'k');
        end
    end
    set(gca,'XLim',[sdfTimes(1),sdfTimes(end)]);
    keyboard
...
%         activeStarts = activeTimes(~isnan(activeTimes(:,1)),1);
        ...
        if length(startTimes) < 5, modLat = nan; return; end
