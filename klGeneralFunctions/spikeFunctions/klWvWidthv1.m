function [wvWidth,tfRepol] = klWvWidthv1(alWaves,alTimes)

visualize = 1;

% Cut down waves and times to times > 0
checkWaves = (alWaves(:,alTimes > 0)-repmat(nanmean(alWaves,2),1,sum(alTimes > 0)))./repmat(nanstd(alWaves,[],2),1,sum(alTimes > 0));
checkTimes = alTimes(:,alTimes > 0);
...
for iw = 1:size(alWaves,1),
    ...
    % positive spike
    if (checkWaves(iw,1) - checkWaves(iw,3)) < 0,
