...
if bootSig
    ...
    for ib = 1:nBoot
        ...
        try
        bootAUC(ib) = klROCv1(testA,testB,'boot',0);      
        catch
            keyboard
        end
    end
    bootP = sum(abs(bootAUC-.5) >= abs(auc-.5))/length(bootAUC);
    % Now sort the bootstrapped AUCs so we can find upper and lower tails
    % (5% total - defaults to two-tailed test)
    sortAUC = sort(bootAUC,'ascend');
    cutLen = floor((alph/2)*length(sortAUC)); % floor to be conservative
