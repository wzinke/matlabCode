...
%         fprintf('Getting random observation %d of %d\n',irand,randReps);
        ...
        switch randValType,
            ...
            case {'diff'},
                % Get point-to-point difference of obs
                dObs = diff(obs,[],2);
                
                % Get a matrix of random values
                switch pcaRandType
                    case {'unif'},
                        randMat = rand(size(obs,1),size(obs,2)-1);
                        minMat = repmat(min(dObs,[],1),size(obs,1),1);
                        maxMat = repmat(max(dObs,[],1),size(obs,1),1);
                        
                        randDiffs = minMat+(randMat.*(maxMat-minMat));
                        randStart = (rand(size(obs,1),1).*(max(obs(:,1))-min(obs(:,1))))+min(obs(:,1));
                    case {'gauss'},
                        randMat = randn(size(obs,1),size(obs,2)-1);
                        mnMat = repmat(nanmean(dObs,1),size(dObs,1),1);
                        stdMat = repmat(nanstd(dObs,[],1),size(dObs,1),1);
                        
                        randDiffs = (randMat.*stdMat)+mnMat;
                        randStart = (randn(size(obs,1),1).*nanstd(obs(:,1),[],1))+nanmean(obs(:,1));
                end
                
                randObs  = [randStart,repmat(randStart,1,size(randDiffs,2))+cumsum(randDiffs,2)];
