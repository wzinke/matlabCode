...
function [kHat, gapVect, s, summaryStruct] = klGetGapv11(obs,groups,varargin)
...
for iv = 1:length(varStrInd),
    switch varargin{varStrInd(iv)},
        ...
        case {'start'},
            startInd = varargin{varStrInd(iv)+1};
    end
end
...
%         fprintf('Getting random observation %d of %d\n',irand,randReps);
        ...
        switch randValType,
            ...
            case {'diff'},
                ...
                for is = 1:length(startInd),
                    if is==length(startInd),
                        theseInds = startInd(is):size(obs,2);
                    else
                        theseInds = startInd(is):(startInd(is+1)-1);
                    end
                    dObs = diff(obs(:,theseInds),[],2);
                       
                    switch pcaRandType
                        case {'unif'},
                            randMat = rand(size(obs,1),length(theseInds)-1);
                            minMat = repmat(min(dObs,[],1),size(obs,1),1);
                            maxMat = repmat(max(dObs,[],1),size(obs,1),1);

                            randDiffs = minMat+(randMat.*(maxMat-minMat));
                            randStart = (rand(size(obs,1),1).*(max(obs(:,startInd(is)))-min(obs(:,startInd(is)))))+min(obs(:,startInd(is)));
                        case {'gauss'},
                            randMat = randn(size(obs,1),length(theseInds)-1);
                            mnMat = repmat(nanmean(dObs,1),size(dObs,1),1);
                            stdMat = repmat(nanstd(dObs,[],1),size(dObs,1),1);

                            randDiffs = (randMat.*stdMat)+mnMat;
                            randStart = (randn(size(obs,1),1).*nanstd(obs(:,startInd(is)),[],1))+nanmean(obs(:,startInd(is)));
                    end

                    randObs(:,theseInds) = [randStart,repmat(randStart,1,size(randDiffs,2))+cumsum(randDiffs,2)];
                end
...
%                 end
                ...
                % New PCA version 1/16/17
                [coeffs,scores,~,~,expl] = pca(obs,'Centered',0);
                randScores = klMakeRand(size(scores,1),nanmean(scores,1),nanstd(scores,[],1));
                randObs = randScores*coeffs';
...
%             randClusts = unique(randClustIDs(:,ik)); randClusts(isnan(randClusts)) = [];
    ...
    s = s(1:find(isfinite(gapVect),1,'last'));
    gapVect = gapVect(1:find(isfinite(gapVect),1,'last'));
...
elseif ismember(randType,{'shuffle'}),
    for ik = 1:size(groups,2)
        clusts = unique(groups(~isnan(groups(:,ik)),ik));
        randD = zeros(randReps,length(clusts));
        for irand = 1:randReps,
            myGroups = klShuffle(groups(:,ik));
            for ic = 1:max(myGroups),
                clustInds = find(myGroups == clusts(ic));
                randN(irand,ic) = length(clustInds);
                randD(irand,ic) = dFromSim(simMat,clustInds);
            end
             Wstar(irand,ik) = sum(randD(irand,:)./(2.*randN(irand,:)));
             sumDstar(irand,ik) = sum(randD(irand,:));
        end
    end
...
if ~ismember(randType,{'rand','shuffle'}),
    for irand = 1:randReps,
