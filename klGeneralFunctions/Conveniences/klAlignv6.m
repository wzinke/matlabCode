function [outMat,currZero] = klAlignv6(inCell,indVect)

% Cut out nans in indVect, but pay attention to where they came from
noNanInd = find(~isnan(indVect));
origSize = size(inCell,1);
inCell = inCell(noNanInd,:);
indVect = indVect(noNanInd);
...
outMatTmp = outMat;
clear outMat;
outMat = nan(origSize,dim2);
outMat(noNanInd,:) = outMatTmp;
