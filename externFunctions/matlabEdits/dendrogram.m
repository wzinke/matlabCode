...
if nargin > 2
    ...
    pnames = {'orientation' 'colorthreshold' 'labels'  'reorder','checkcrossing','display'};
    dflts =  {orientation   'default'        obslabels leafOrder, true, display};
    [orientation,threshold,obslabels,leafOrder,check,display,setFlag] =  ...
        internal.stats.parseArgs(pnames, dflts, varargin{1+offset:end});
    
    if ~isempty(display) && ischar(display),
        if strcmp(display,'off'), display = 0; else display = 1; end
