...
for it = 1:nTrs,
    ...
    LFP.visInds(it,1:sum(lfpTimes >= (Task.AlignTimes(it)+lfpWind(1)) & lfpTimes <= (Task.AlignTimes(it)+lfpWind(2)))) = find(lfpTimes >= (Task.AlignTimes(it)+lfpWind(1)) & lfpTimes <= (Task.AlignTimes(it)+lfpWind(2)));
end
