function [outSorts] = tdtSortChanv5a(chanData,varargin)
...
[spkTimes,spkWaves,spkPols,spkThresh] = klThreshCrossv6(chanData,'times',chanTimes,'-m',minRefract,'-t',-thisThresh);
spkTimesN = spkTimes(spkPols==2);
spkTimesP = spkTimes(spkPols==1);
spkWavesN = spkWaves(spkPols==2,:);
spkWavesP = spkWaves(spkPols==1,:);

% clear chanData
chanData = [];

outSorts.neg = tdtSortWavesv1a(spk
% Get a subset of the waveforms
if length(spkTimesN) <= maxWvs,
    subIndsN = 1:length(spkTimesN);
else
