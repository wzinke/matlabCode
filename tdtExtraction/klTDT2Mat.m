function [Task, spiketimes, LFP] = klTDT2Mat(filepath,varargin),
...
Task = decodeEvents(infoMat,trialCodes,trialTimes);
...
%% Finally, place LFPs
if hasLFP,
    LFP = nan(nTrs,(length(lfpWind(1):lfpRate:lfpWind(2)))+1);
    for ic = doChans,
        for it = 1:nTrs,
            LFP(it,1:sum(lfpTimes >= (Task.AlignTimes(it)+lfpWind(1)) & lfpTimes <= (Task.AlignTimes(it)+lfpWind(2)))) = tdtLFP(ic,lfpTimes >= (Task.AlignTimes(it)+lfpWind(1)) & lfpTimes <= (Task.AlignTimes(it)+lfpWind(2)));
        end
    end
end
