function tdtSortSessionv2_Parallel(inFile,varargin)
...
%     clear chanSEVs chanSorts
    ...
    try
        if manThresh,
            allSorts(ic) = tdtSortChanv5a(chanSEVs.Wav1.data,'-t',chanThresh(ic));
        else
            allSorts(ic) = tdtSortChanv5a(chanSEVs.Wav1.data);
        end
...
for ic = 1:numChans,
    chanSorts = allSorts(ic);
    saveStr = sprintf('%s/%s/Channel%d/autoSortAgglom_noAudit.mat',procDir,inFile,ic);
    save(saveStr,'chanSorts');
    allSorts(ic) = [];
end
