...
% Set constants
lfpWind = [-500, 6500];
lfpFreq = 1000/1073; % 1000 converts to ms
% spkFreq = 1/
...
for is = 1:length(toDo),
    ...
    for ic = 1:numChans,
        ...
        % Read the data
        chanSEVs = SEV2mat(sprintf('%s/%s',rawDir,thisFile),'CHANNEL',ic,'VERBOSE',0);
        spkTimesRaw = 0:(1000/chanSEVs.Wav1.fs):((1000/chanSEVs.Wav1.fs)*(size(chanSEVs.Wav1.data,2)-1)); % 1000x multiplier converts to ms
        ...
        if exist('sortedChans','var'),
            ...
            [spkTimes,spkWaves,spkThresh] = klThreshCrossv3(chanSEVs.Wav1.data(ic,:),'times',spkTimesRaw);
        end
        ...
        % Now get LFP
        LFP.vis = nan(nTrs,(length(lfpWind(1):lfpFreq:lfpWind(2)))+1);
        for it = 1:nTrs,
        ...
        LFP.vTimes = ((1:size(LFP.vis,2))+lfpWind(1)).*lfpFreq;
        ...
        save(sprintf('%s/%s/Channel%d/Waves.mat',procDir,thisFile,ic),'spkWaves');
