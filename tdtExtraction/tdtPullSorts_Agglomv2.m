function tdtPullSorts_Agglomv2(file)
        ...
        switch compType,
            ...
            case 'euc', % Get minimum euclidean distance from centroid
                ...
                for ik = 1:chanSorts.(myPols{ip}).k,
                    clustDist = EuDist2(subScores(chanSorts.(myPols{ip}).idx==ik,1:2));
                    maxDists(ik) = max(clustDist(:));
                end
                
                polTimes = chanSorts.(myPols{ip}).allTimes';
                polID = ones(length(polTimes),1).*ip;
                if chanSorts.(myPols{ip}).k == 1,
                    polIDX = ones(size(allScores,1),1);
                else
                    switch linkType,
                        case {'single','min'},
                            % Loop through remaining scores
                            polIDX = uint16(zeros(size(allScores,1),1));
                            for iRep = 1:ceil(size(allScores,1)/maxWvs),
                                repInds = (1:maxWvs)+(maxWvs.*(iRep-1));
                                repInds(repInds > size(allScores,1)) = [];
                                repDist = EuDist2(subScores(:,1:2),allScores(repInds,1:2));
                                [minVals,minInd] = min(repDist,[],1);
                                for ik = 1:chanSorts.(myPols{ip}).k,
                                    minInd(minInd==ik & minVals > maxDists(ik)) = max(chanSorts.(myPols{ip}).idx)+1;
                                end
                                polIDX(repInds) = chanSorts.(myPols{ip}).idx(minInd);
                            end
                        case {'average','mean'},
