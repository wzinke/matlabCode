function klDoBradenSEFBookKeeping(task,varargin)

switch task
    case {'MG','mg'}
        outFile = './klBradenSEFBookKeeping_mg.xlsx';
        sessHead = 'MG';
    case {'Search','Capture','search','capture','cap','SAT','sat'}
        outFile = './klBradenSEFBookKeeping_search.xlsx';
        sessHead = 'SEARCH';
end
...
for im = 1:length(monks)
    ...
    allSess = dir([tebaBase,filesep,monks{im},filesep,'SEF Popout/Matlab/Final sorts',filesep,upper(monks{im}(1)),'*',sessHead,'*']);
...
%     allSess = [satSess;satSefSess];
    ...
    fprintf('Analyzing Braden data for %s...(%d sessions)\n',monks{im},length(allSess));
    ...
    for is = 1:length(allSess)
        ...
        waveFile=dir([tebaBase,filesep,monks{im},'/SEF Popout/Matlab/Final sorts/waves/',allSess(is).name(1:(strfind(allSess(is).name,'-')-1)),'*']);
        if ~isempty(waveFile)
            load([waveFile(1).folder,filesep,waveFile(1).name]);
        end
        ...
        for iu = 1:length(sessUnits)
            ...
            % Get SNR
            if exist(sprintf('waves_%s',sessUnits(iu).name),'var')
                eval(['snr = klGetSNRv1(waves_',sessUnits(iu).name,');']);
            else
                snr = nan;
            end
...
%             [~,whichFold] = fileparts(fileparts(allSess(is).folder));
            ...
            outRow = {allSess(is).name,thisChan,thisUnit,hasOther,blRate,mnRate,percISI,snr,rayV,rayM,vrf,mrf,mnAUC,tst};
