...
if sum(isnan(spikes(:))) == numel(spikes),
    ...
    latencies.vis.targLocLat = nan;
    latencies.vis.targTypeLat = [nan,nan];

    latencies.mov.sig = nan;
    latencies.mov.rise = nan;
    latencies.mov.poissLat = nan;
    latencies.mov.percPoiss = nan;
    latencies.mov.targLocLat = nan;
    latencies.mov.targTypeLat = [nan,nan];
...
[mSDF,mTimes] = klSpkRatev2(spikes-repmat(capTask.SRT+capTask.GoCue,1,size(spikes,2)),'-q',1);
