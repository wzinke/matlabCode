function [grpSDF, grpSDFTimes, grpWaves, grpWvTimes, normMat, grpStd] = klGetFilev8(xlRows,varargin)
...
for ir = 1:length(xlRows),
    ...
    Task=klCutTask(Task,ismember(Task.TaskType,task));
    spiketimes = spiketimes(ismember(Task.TaskType,task),:);
    ...
    if doRF || doDiff,
        vCrit = Task.TargetLoc == vRF & Task.Correct == 1;
        mCrit = Task.TargetLoc == mRF & Task.Correct == 1;
        rCrit = (Task.TargetLoc == vRF | Task.TargetLoc == mRF)  & Task.Correct == 1;
        switch diffType,
            case {'arf'},
        ...
        vCrit =  Task.Correct == 1;%true(size(spiketimes,1),1);
        mCrit =  Task.Correct == 1;%true(size(spiketimes,1),1);
        rCrit = Task.Correct == 1;
    end
    ...
    [rawRTSDF,grpSDFTimes{ir,4}] = klSpkRatev2(spiketimes-repmat(Task.RewardTone,1,size(spiketimes,2)),'-q',1);
    [rawRSDF,grpSDFTimes{ir,5}] = klSpkRatev2(spiketimes-repmat(Task.Reward,1,size(spiketimes,2)),'-q',1);
    if clip,
    ...
    if doDiff,
        ...
        grpSDF{ir,4} = nanmean(rawRTSDF(rCrit,:),1);
        grpSDF{ir,5} = nanmean(rawRSDF(rCrit,:),1);
    end
