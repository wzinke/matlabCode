...
for ir = 1:length(newDates),
    outRow = {upper(monk(1)),newDates{ir},'028','cosman',task{ir},notes{ir},newFiles{ir},'FEF','right'};
    outMat = cat(1,outMat,outRow);
end

% Today's excel file name is...
thisDate = clock;
xlFile = sprintf('fileTableUpdates_%d%d%d.xlsx',thisDate(1),thisDate(2),thisDate(3));

xlswrite(xlFile,outMat,monk);
