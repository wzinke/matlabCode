...
for ir = 1:length(xlRows),
    ...
    if sum(vCrit), [rawVisSDF,grpSDFTimes{ir,3}] = klSpkRatev2(spiketimes(vCrit,:),'-q',1); else rawVisSDF = nan; grpSDFTimes{ir,3} = nan; end
    if sum(mCrit), [mSDF, grpSDFTimes{ir,2}] = klSpkRatev2(spiketimes(mCrit,:)-repmat(Task.SRT(mCrit,:)+Task.GoCue(mCrit,:),1,size(spiketimes,2)),'-q',1); else mSDF = nan; grpSDFTimes{ir,2} = nan; end
    if clip,
        spiketimes(spiketimes > repmat(saccTime,1,size(spiketimes,2))) = nan;
    end
    if sum(vCrit), [vSDF, grpSDFTimes{ir,1}] = klSpkRatev2(spiketimes(vCrit,:),'-q',1); else vSDF = nan; grpSDFTimes{ir,1} = nan; end
