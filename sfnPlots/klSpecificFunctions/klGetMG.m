...
if sum(isnan(spikes(:))) == numel(spikes),
    myType = 'na';
    tuneParams.vis = struct('mu',nan,'sig',nan,'amp',nan,'bl',nan);
    tuneParams.mov = struct('mu',nan,'sig',nan,'amp',nan,'bl',nan);
    
    % Make latency structure
    latencies.vis.sig = nan;
    latencies.vis.rise = nan;
    latencies.vis.poissLat = nan;
    latencies.vis.percPoiss = nan;
    
    latencies.mov.sig = nan;
    latencies.mov.rise = nan;
    latencies.mov.poissLat = nan;
    latencies.mov.percPoiss = nan;
...
% Make SDFs
[vSDF,vTimes] = klSpkRatev2(spikes,'-q',1);
[mSDF,mTimes] = klSpkRatev2(spikes-repmat(mgTask.SRT+mgTask.GoCue,1,size(spikes,2)),'-q',1);
...
if sum(isnan(vLocMean)) > 4 || sum(isnan(mLocMean)) > 4,
    myType = 'na';
    tuneParams.vis = struct('mu',nan,'sig',nan,'amp',nan,'bl',nan);
    tuneParams.mov = struct('mu',nan,'sig',nan,'amp',nan,'bl',nan);
...
sortLocs = sort(uLocs);
if ~isnan(vRF),
    if vRF > 315, vLocs(1) = 0; else vLocs(1) = sortLocs(find(sortLocs >= vRF,1)); end
...
if ismember(myType,{'vis','none'}),
    mPoiss = nan; mSig = nan; mRise = nan; poissMovMat = nan;
end
if ismember(myType,{'mov','none'}),
    vPoiss = nan; vSig = nan; vRise = nan; poissVisMat = nan;
end
