...
function [modLat, activeTimes] = poissLat_Longv3(spiketimes,varargin)
% Set some defaults
sigAlph = .005;
checkAlph = .01;
visualize = 0;
...
for it = 1:size(spiketimes,1),
    ...
    trMeanRate(it) = length(thisTrSpks)/(rwdTimes(it)-(-500));
    
    % Index to first spike after target presentation
    firstSpkInd = find(thisTrSpks >= 0,1);
...
[sortStarts, sortInds]   = sort(activeStarts,'ascend');
...
modLat = (sortStarts(maxInd)+sortStarts(maxInd+J))/2;
