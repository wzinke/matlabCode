function [respK, respClustIDs, respGap, respGapErr, linkMat, linkInd, thisDistOut, respInOut] = klRespClustForMetaclustv3(allSDF,allTimes,varargin)
...
if ~any(size(allSDF)==1),
    ...
    for i = 1:size(allSDF,2)
        respAlign{i} = allSDF{i};
        respTimes{i} = allTimes{i};
    end
end
...
normResp = klNormRespv2(respAlign,respTimes,respNormType,'-r',respWinds,'bl',blWind,'sd',normSD);

% Cut down the responses to relevant times and to get rid of nans
catNorms = [];
for i = 1:length(normResp)
    catNorms = cat(2,catNorms,normResp{i}(:,ismember(respTimes{i},respWinds{i})));
end
...
for i = 1:length(allEpocs)
    ...
    epocTms{i} = respTimes{epocInds(i)};
end
...
convResp = [];
for i = 1:length(normResp)
    convResp = cat(2,convResp,conv2(normResp{i}(:,ismember(respTimes{i},respWinds{i})),kern,'same'));
end
startInds = [1,length(1:nSkip:((size(normResp{1},2)/2)))+1];
...
if distOnly
    [respK, respClustIDs, respGap, respGapErr, linkMat, linkInd, respInOut] = deal(nan);
    return;
end
